...
public class ProjectileExplosion extends Explosion {
    ...
    public void doExplosionA() {
        ...
        List<Entity> list = this.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ));
        ...
        for (Entity entity : list) {
            if (!entity.isImmuneToExplosions()) {
                ...
                if (distanceToExplosion <= this.radius) {
                    ...
                    double deltaDistance = MathHelper.sqrt(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ);
                    if (deltaDistance != 0.0D) {
                        deltaX = deltaX / deltaDistance;
                        deltaY = deltaY / deltaDistance;
                        deltaZ = deltaZ / deltaDistance;

                        double density = this.world.getBlockDensity(vec3d, entity.getEntityBoundingBox());
