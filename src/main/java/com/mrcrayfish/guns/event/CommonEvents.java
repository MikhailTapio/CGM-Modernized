...
public class CommonEvents {
    ...
    private static class ReloadTracker {
        ...
        public void increaseAmmo(EntityPlayer player) {
            AmmoContext ammo = ItemGun.findAmmo(player, gun.projectile.item);
            final ItemStack ammoStack = ammo.ammo();
            if (!ammoStack.isEmpty()) {
                int amount = Math.min(ammoStack.getCount(), gun.general.reloadSpeed);
                NBTTagCompound tag = stack.getTagCompound();
                if (tag != null) {
