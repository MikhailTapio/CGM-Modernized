...
import com.mrcrayfish.guns.compat.V0idBackpacksHelper;
import com.mrcrayfish.guns.entity.EntityProjectile;
import com.mrcrayfish.guns.event.CommonEvents;
import com.mrcrayfish.guns.init.ModCompatibilities;
...
public class ItemGun extends ItemColored {
    ...
    public static AmmoContext findAmmo(EntityPlayer player, ResourceLocation id) {
        if (player.capabilities.isCreativeMode) {
            ...
            return ammo != null ? AmmoContext.of(new ItemStack(ammo, 64)) : AmmoContext.NONE;
        }
        for (int i = 0; i < player.inventory.getSizeInventory(); ++i) {
            ItemStack stack = player.inventory.getStackInSlot(i);
            if (isAmmo(stack, id)) {
                return AmmoContext.of(stack);
            }
        }
        AmmoContext ctx = AmmoContext.NONE;
        if (ModCompatibilities.isV0idSmartBackpacksLoaded()) {
            ctx = V0idBackpacksHelper.findAmmo(player, id);
        }
        return ctx;
    }

    public static boolean isAmmo(ItemStack stack, ResourceLocation id) {
        return stack != null && id.equals(stack.getItem().getRegistryName());
    }
